...
@section('head')
    <style>
        ...
        #mermaid g.node rect:hover {
            fill: rgb(240, 240, 247);
            stroke: #ff967b;
        }

        /* https://unpkg.com/mermaid@7.0.5/dist/mermaid.min.css */
    ...
    <script>
        ...
        function table_clicked(tbl) {
            ...
            req = $.get('/dbtolaravel/{{ $connection }}/' + tbl + '/infos').done(function(data) {
                ...
                infos.jsonViewer(data);
                migration.text(data.migration.content);
                routes.text(data.routes.content);
                controller.text(data.controller.content);
                model.text(data.model.content);
                ...
                blades.text('<!-- view.blade.php -->\n' + data.blades.view.content +
                    '\n\n<!-- edit.blade.php -->\n' + data.blades.edit.content +
                    '\n\n<!-- list.blade.php -->\n' + data.blades.list.content);
        ...
        function writefile(key, overwrite) {
            ...
            $.ajax({
                ...
                data: {file: document.infos[key].path},
                method: 'PUT'
            }).done(function(data) {
        ...
        function showDBInfos() {
            ...
            $.get('/dbtolaravel/{{ $connection }}/infos', function (data) {
                ...
                var graph = mermaidAPI.render('graphDiv', str, insertSvg);
                ...
                $('#mermaid g.node').click(function(ele) {
                    ele = $(ele.currentTarget);
                    table_clicked(ele.find('g.label > g div').text());
                });
            });
        }
        hljs.initHighlightingOnLoad();
        setTimeout(showDBInfos, 10);
    </script>
@endsection
